...
namespace M15_TrabalhoModelo_2021_22
{
    class Utils
    {
        ...
        static public string pastaDoPrograma()
        {
            ...
            pastaInicial += @"\M15_TrabalhoModelo";
        ...
        /// <summary>
        /// Versão para datatable na datagrid
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="dataGrid"></param>
        /// <param name="title"></param>
        static public void printDG2<T>(DataGrid dataGrid, string title)
        {
            PrintDialog printDialog = new PrintDialog();
            ...
            if (printDialog.ShowDialog() == true)
            {
                ...
                for (int i = 0; i < dataGrid.Items.Count; i++)
                {
                    ...
                    if (dataGrid.ItemsSource.ToString().ToLower() == "system.data.dataview")
                    {
                        row = (DataRowView)dataGrid.Items.GetItemAt(i);
                    }
                    else
                    ...
                    {
                        for (int j = 0; j < row.DataView[i].Row.ItemArray.Length; j++)
                        {
                            r.Cells.Add(new TableCell(new Paragraph(new Run(row.Row.ItemArray[j].ToString()))));
